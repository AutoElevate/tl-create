...
class Microsoft {
  getTrusted(data, skipFetch = false) {
    ...
    else databuf = Buffer.from(data, "binary");
    let variant;
    for (let i = 0; i < databuf.buffer.byteLength; i++) {
      variant = asn1js.verifySchema(databuf.buffer.slice(i), ctl_schema);
      if (variant.verified === true) break;
    }
    if (variant.verified === false) throw new Error("Cannot parse STL");
    if (!skipFetch) process.stdout.write("Fetching certificates");
    for (let ctlentry of variant.result.CTLEntry) {
      if (!skipFetch) process.stdout.write(".");
      let ctlentry_parsed = asn1js.verifySchema(
        ctlentry.toBER(),
        ctlentry_schema
      );
      let certid = pvutils.bufferToHexCodes(
        ctlentry_parsed.result.CertID.valueBlock.valueHex
      );
      let certraw = "";
      if (!skipFetch) certraw = this.fetchcert(certid);
      let tl_cert = {
        raw: certraw,
        trust: [],
        operator: "",
      ...
      for (let metadata of ctlentry_parsed.result.CertMetaData) {
        ...
        if (metadata_oid === "1.3.6.1.4.1.311.10.11.9") {
          let ekus = asn1js.verifySchema(
            metadata.valueBlock.value[1].valueBlock.value[0].valueBlock
              .valueHex,
            eku_schema
          );
          for (let eku of ekus.result.OID) {
            ...
            if (eku_oid in EKU_oids)
              (_a = tl_cert.trust) === null || _a === void 0
                ? void 0
                : _a.push(EKU_oids[eku_oid]);
          }
        }
        if (metadata_oid === "1.3.6.1.4.1.311.10.11.11") {
          tl_cert.operator = String.fromCharCode
            .apply(
              null,
              new Uint16Array(
                metadata.valueBlock.value[1].valueBlock.value[0].valueBlock.valueHex
              )
            )
            .slice(0, -1);
        }
        if (metadata_oid === "1.3.6.1.4.1.311.10.11.83") {
          let evoids = asn1js.verifySchema(
            metadata.valueBlock.value[1].valueBlock.value[0].valueBlock
              .valueHex,
            evoid_schema
          );
          for (let evoid of evoids.result.PolicyThing) {
            (_b = tl_cert.evpolicy) === null || _b === void 0
              ? void 0
              : _b.push(evoid.valueBlock.value[0].valueBlock.toString());
          }
        }
      }
      tl.AddCertificate(tl_cert);
    }
    if (!skipFetch) console.log();
    return tl;
  }
  getDisallowed(data, skipFetch = false) {
    ...
    if (!data)
      databuf = this.fetchSTL(
        microsoftDisallowedURL,
        microsoftDisallowedFilename
      );
    else databuf = Buffer.from(data, "binary");
    let variant;
    for (let i = 0; i < databuf.buffer.byteLength; i++) {
  ...
  fetchSTL(uri, filename) {
    let res = sync_request_1.default("GET", uri, {
      timeout: 10000,
      retry: true,
      headers: { "user-agent": "nodejs" },
    });
    ...
    if (process.platform === "win32")
      ...
      child_process.execSync("cabextract " + filename + ".cab", {
        cwd: dirpath,
      });
