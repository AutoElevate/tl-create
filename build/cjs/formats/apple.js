...
class Apple {
    ...
    getTrusted(dataTlList, dataCertList, dataEvRoots, skipFetch = false, {} = {}) {
    ...
    getLatestVersion(data = "") {
        if (!data) {
            let res = sync_request_1.default("GET", this.url, { "timeout": 10000, "retry": true, "headers": { "user-agent": "nodejs" } });
    ...
    getTrustedCertList(version, data = "") {
        if (!data) {
            ...
            let res = sync_request_1.default("GET", url, { "timeout": this.timeout, "retry": true, "headers": { "user-agent": "nodejs" } });
            data = res.body.toString();
        }
        let ch = cheerio_1.default.load(data);
        let filenames = [];
